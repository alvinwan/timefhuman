...
from timefhuman.extraction import extract_fast
from timefhuman.fastpath import parse_fast
from timefhuman.main import _parse_lalr, tfhConfig
...
def main():
    print(f"{'expression case':18} {'timefhuman':>12} {'parse_fast':>12} {'parse_lalr':>12}")
    for label, text in EXPRESSION_CASES:
        print(
            f"{label:18} "
            f"{bench(lambda: timefhuman(text, config=CONFIG)):12.1f} "
            f"{bench(lambda: parse_fast(text, CONFIG, TIMEZONE_MAPPING)):12.1f} "
            f"{bench(lambda: _parse_lalr(text, CONFIG)):12.1f}"
        )

    print()
    print(f"{'extract case':18} {'timefhuman':>12} {'fast extract':>12} {'lalr extract':>12}")
    for label, text in EXTRACT_CASES:
        print(
            ...
            f"{bench(lambda: extract_fast(text, lambda candidate, start: _parse_lalr(candidate, CONFIG, start)), iterations=2000):12.1f}"
        )
